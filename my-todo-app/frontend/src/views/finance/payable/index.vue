<!-- views/finance/payable/index.vue - 应付账款管理页面 -->
<template>
  <div class="payable-container">
      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="totalAmount" label="应付金额" width="120">
          <template #default="{ row }">
            <span style="color: #e6a23c; font-weight: bold;">¥ {{ row.totalAmount?.toFixed(2) || '0.00' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="paidAmount" label="已付金额" width="120">
          <template #default="{ row }">
            <span style="color: #67c23a;">¥ {{ row.paidAmount?.toFixed(2) || '0.00' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="remainingAmount" label="剩余金额" width="120">
          <template #default="{ row }">
            <span>¥ {{ (row.totalAmount - row.paidAmount)?.toFixed(2) || '0.00' }}</span>
          </template>
        </el-table-column>
    ...
    >
      <el-form ref="payFormRef" :model="payForm" :rules="payRules" label-width="100px">
        ...
        <el-form-item label="已付金额">
          <el-input :value="`¥ ${currentRow?.paidAmount?.toFixed(2) || '0.00'}`" disabled />
        </el-form-item>
        <el-form-item label="剩余金额">
          <el-input :value="`¥ ${(currentRow?.totalAmount - currentRow?.paidAmount)?.toFixed(2) || '0.00'}`" disabled />
        </el-form-item>
        <el-form-item label="本次付款" prop="amount">
          <el-input-number v-model="payForm.amount" :min="0.01" :max="currentRow?.totalAmount - currentRow?.paidAmount" :precision="2" :step="100" style="width: 100%;" />
        </el-form-item>
        <el-form-item label="付款备注" prop="remark">
          <el-input v-model="payForm.remark" type="textarea" :rows="2" placeholder="请输入付款备注" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="payDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handlePaySubmit" :loading="payLoading">确认付款</el-button>
      </template>
    </el-dialog>
    ...
    >
      <el-table :data="overdueList" border stripe>
        ...
        <el-table-column prop="remainingAmount" label="剩余金额" width="120">
          <template #default="{ row }">
            ¥ {{ (row.totalAmount - row.paidAmount)?.toFixed(2) || '0.00' }}
          </template>
        </el-table-column>
        <el-table-column prop="dueDate" label="到期日期" width="120" />
...
import { getPayablePage, createPayable, makePayment, getOverduePayables, deletePayable } from '@/api/finance'
...
const handleView = (row) => {
  ElMessageBox.alert(`
    <p><strong>供应商名称：</strong>${row.supplierName}</p>
    <p><strong>采购单号：</strong>${row.orderNo || '-'}</p>
    <p><strong>应付金额：</strong>¥ ${row.totalAmount?.toFixed(2) || '0.00'}</p>
    <p><strong>已付金额：</strong>¥ ${row.paidAmount?.toFixed(2) || '0.00'}</p>
    <p><strong>剩余金额：</strong>¥ ${(row.totalAmount - row.paidAmount)?.toFixed(2) || '0.00'}</p>
...
const handlePaySubmit = async () => {
  ...
  await payFormRef.value.validate(async (valid) => {
    ...
    try {
      await makePayment(currentRow.value.id, payForm.amount)
...
const handleDelete = async (row) => {
  await ElMessageBox.confirm('确定要删除该应付账款吗?', '提示', { type: 'warning' })
  try {
...
const handleShowOverdue = async () => {
  try {
    const res = await getOverduePayables()
    overdueList.value = res || []
    if (overdueList.value.length === 0) {
      ElMessage.info('暂无逾期应付账款')
    } else {
